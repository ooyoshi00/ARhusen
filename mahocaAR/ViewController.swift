...
class ViewController: UIViewController, ARSCNViewDelegate, ARSessionDelegate {
    ...
    @IBOutlet weak var bd_onoff: UIButton!
...
//        initialize()
        ...
        bd_onoff.setImage(UIImage.init(named: "onoff"), for: UIControl.State.normal)
...
//                    let node=nodes
                    ...
                    //回転
                    node.rotation=SCNVector4(1, 0, 0, 1.0 * Double.pi)
//                    node.rotation=SCNVector4(0, 0, 1, 1.0 * Double.pi)
                    ...
                    //位置
                    //位置
                    print("ポジション：",self.hitTestposition)
                    print("hithit")
                    guard let transform2 = self.hitTestposition else{
                        print("hitposition")
                        return
                    }
                    let transform = transform2.worldTransform
...
//    }
    //ボタンビューの表示切替
    @IBAction func bd_onoff(_ sender:Any){
        button_view.isHidden = !button_view.isHidden
//        print(button_view.isHidden)
//        UIView.transition(with: button_view, duration: 1.0, options: [.transitionCurlUp, .autoreverse], animations: {
//            self.button_view.isHidden = true
//        }) { _ in
//            self.button_view.isHidden = false
//        }
//        UIView.animate(withDuration:1.0,options: .autoreverse,animations: {
//            button_view.isHidden = !button_view.isHidden
//        })
    }
    //写真追加のボタン
...
//        let naviVC = UINavigationController(rootViewController:nextvc)
    ...
    @objc func tapped(sender: UITapGestureRecognizer) {
        ...
        // タップされた位置のARアンカーを探す
        var hitTestResult = sceneView.hitTest(tapLocation, types: .existingPlaneUsingExtent)
        ...
        if !hitTestResult.isEmpty && oppButton{
...
//            let hitResults = sceneView.hitTest(p, options: [:])
            //hitTestResult = sceneView.hitTest(sender.location(in: sceneView), options:[:])
            
            self.hitTestposition = hitTestResult
            print("ヒットテスト")
            ...
            if firstNode?.childNodes != []{
                //print("子ノード：",firstNode?.childNodes)
                let title = "付箋を選択しました"
                let mes = "ok"
                alertMessage(title:title, message: mes)
            }
